...
import (
	...
	"ProjectDelivery/component/tokenprovider"
...
type loginBusiness struct {
	...
	tokenProvider tokenprovider.Provider //chỉ xài interface nen có thể unittest vs token cứng  cũng dc .lợi ích xài interface
	//lúc này provider chỉ là interface và thằng inplement nó là jwt trong cùng thư mục
	hasher        Hasher
	expiry int
}
...
func (business *loginBusiness) Login(ctx context.Context, data *usermodel.UserLogin) (*tokenprovider.Token, error) {
	...
	//refreshToken, err := business.tokenProvider.Generate(payload, business.tkConfig.GetAtExp() *2)
	//if err != nil {
	//	return nil, common.ErrInternal(err)
	//}
